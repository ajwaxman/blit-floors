...
export const fetchUnder100 = async () => {
    ...
    const mappedMetadata = flatten(terraformMetadata)
        .filter((t) => {
            return t.seedValue < 100
        })

    _.forEach(mappedMetadata, function(t){
        under100.push(t.tokenId);
    })

    const chunkedUnder100 = chunk(under100, 20)

    const data = await pMap(chunkedUnder100, fetchTerraformPage, { concurrency: 2 })
