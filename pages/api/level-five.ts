...
export const fetchLevelFive = async () => {
    ...
    _.forEach(mappedMetadata, function(t){
        levelFive.push(t.tokenId);
    })

    const chunkedLevelFive = chunk(levelFive, 20)

    const data = await pMap(chunkedLevelFive, fetchTerraformPage, { concurrency: 2 })
