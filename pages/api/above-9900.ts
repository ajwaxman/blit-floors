...
export const fetchAbove9900 = async () => {
    ...
    const mappedMetadata = flatten(terraformMetadata)
        .filter((t) => {
            return t.seedValue > 9900
        })

    _.forEach(mappedMetadata, function(t){
        above9900.push(t.tokenId);
    })

    const chunkedAbove9000 = chunk(above9900, 20)

    const data = await pMap(chunkedAbove9000, fetchTerraformPage, { concurrency: 2 })
